        ...
        if (!(e.target.id == 'zdic_content' || $('#zdic_content').find(e.target).length > 0)) {
            ...
            if (word.length > 0) {
                ...
                $('#zdic_search_img').mouseover(function () {
                    $('#zdic_search').css({'left': e.pageX + 7,'top': e.pageY - 7}).fadeOut(2000, function () {
                        $(this).remove();
                    });
                    $('#zdic_content').draggable().fadeTo(100, 1.0).css({'left': e.pageX + 18, 'top': e.pageY - 7}).niceScroll({cursorborderradius: '0px', cursorwidth: "0px", cursorborder: '0 solid #cccccc', cursorcolor: '#cccccc'});
                    if (word.length == 1) {
                        dict(word);
                    }
                    if (word.length > 1) {
                ...
                $('#zdic_content').fadeOut(100, function () {
                    $(this).remove();
                });
            }
        }
